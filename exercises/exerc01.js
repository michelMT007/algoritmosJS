...
const impares = randomNumbers.filter(index => index%2 !==0)
console.log(impares);
...
const menor = crazyNumbers.filter(index => index < 501)
console.log('No Array há '+menor.length+' menor que 501');
...
const numbers = [5, 7, 3]
const aoquadrado = numbers.map(index => index **2)
...
const antes = tarantinoMovies.filter(tarantinoMovie => tarantinoMovie.release < 2000)
...
const nomeFilm = tvShows.map(tvShow => tvShow.name)
...
const nomeJogo = cart.map(nome => (console.log('- '+nome.name+`${'\n'}`)))
